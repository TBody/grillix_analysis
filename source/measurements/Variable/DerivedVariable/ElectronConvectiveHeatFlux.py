...
from ..BaseVariable import Density, ElectronTemperature

class ElectronConvectiveHeatFlux(DerivedVariable):
    
    def __init__(self, run=None):
        title = "Electron convective heat flux"
        self.vector_variable = True
        self.density = Density()
        self.electron_temperature = ElectronTemperature()
        self.total_velocity = TotalVelocity()
        
        self.base_variables = [self.density, self.electron_temperature, self.total_velocity]
    ...
    def fetch_values(self, **kwargs):
        ...
        electron_convective_flux = density[..., np.newaxis] * electron_temperature[..., np.newaxis] * total_velocity

        return self.normalised_VectorArray(electron_convective_flux)
